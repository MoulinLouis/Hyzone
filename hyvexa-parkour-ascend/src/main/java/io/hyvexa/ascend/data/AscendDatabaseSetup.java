...
public final class AscendDatabaseSetup {
        ...
        try {
            DatabaseManager.withTransaction(conn, c -> {
                try (Statement stmt = c.createStatement()) {
                    ...
                    if (columnExists(c, "ascend_player_maps", "multiplier_value")
                        && columnExists(c, "ascend_player_maps", "robot_multiplier_bonus")) {
                        stmt.executeUpdate("""
                    ...
                    if (columnExists(c, "ascend_player_maps", "robot_count")) {
                        stmt.executeUpdate("""
                            UPDATE ascend_player_maps
    ...
    private static void ensureProgressColumns(Connection conn) {
        ...
        if (!columnExists(conn, "ascend_players", "total_coins_earned")
                ...
                && !columnExists(conn, "ascend_players", "total_volt_earned_mantissa")) {
            try (Statement stmt = conn.createStatement()) {
        ...
        if (!columnExists(conn, "ascend_players", "summit_accumulated_coins")
                ...
                && !columnExists(conn, "ascend_players", "summit_accumulated_volt_mantissa")) {
            try (Statement stmt = conn.createStatement()) {
        ...
        if (!columnExists(conn, "ascend_players", "elevation_accumulated_coins")
                ...
                && !columnExists(conn, "ascend_players", "elevation_accumulated_volt_mantissa")) {
            try (Statement stmt = conn.createStatement()) {
    ...
    private static void migrateToScientificNotation(Connection conn) {
        ...
        if (columnExists(conn, "ascend_players", "volt_mantissa")
                || columnExists(conn, "ascend_players", "vexa_mantissa")
            ...
            DatabaseManager.withTransaction(conn, c -> {
                try (Statement stmt = c.createStatement()) {
                    ...
                    for (String[] col : playerColumns) {
                        ...
                        if (!columnExists(c, "ascend_players", oldCol)) {
                            // Column doesn't exist yet (fresh install or already dropped) — ensure new columns exist
                            if (!columnExists(c, "ascend_players", mantissaCol)) {
                    ...
                    if (columnExists(c, "ascend_player_maps", "multiplier")
                        && !columnExists(c, "ascend_player_maps", "multiplier_mantissa")) {
                    ...
                    } else if (!columnExists(c, "ascend_player_maps", "multiplier_mantissa")) {
            ...
            DatabaseManager.withTransaction(conn, c -> {
                try (Statement stmt = c.createStatement()) {
                    for (String[] rename : renames) {
                        if (columnExists(c, "ascend_players", rename[0])
                                && !columnExists(c, "ascend_players", rename[1])) {
