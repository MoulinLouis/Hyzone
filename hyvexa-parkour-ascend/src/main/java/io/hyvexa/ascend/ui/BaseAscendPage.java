...
public abstract class BaseAscendPage extends InteractiveCustomUIPage<ButtonEventData> {
    ...
    protected static void navigateBackToProfile(Ref<EntityStore> ref, Store<EntityStore> store,
                                                AscendMenuNavigator menuNavigator, BaseAscendPage fallbackPage) {
        Player player = store.getComponent(ref, Player.getComponentType());
        PlayerRef playerRef = store.getComponent(ref, PlayerRef.getComponentType());
        if (player != null && playerRef != null && menuNavigator != null) {
            player.getPageManager().openCustomPage(ref, store, menuNavigator.createProfilePage(playerRef));
        } else {
            fallbackPage.close();
        }
    }
