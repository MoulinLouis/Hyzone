...
class AscendConstantsEconomyTest {
    ...
    void getMapBaseRunTimeMsReturnsCorrectValuesForAllOrders() {
        assertEquals(5000L, AscendConstants.getMapBaseRunTimeMs(0));
        assertEquals(10000L, AscendConstants.getMapBaseRunTimeMs(1));
        assertEquals(16000L, AscendConstants.getMapBaseRunTimeMs(2));
        assertEquals(26000L, AscendConstants.getMapBaseRunTimeMs(3));
        assertEquals(42000L, AscendConstants.getMapBaseRunTimeMs(4));
        assertEquals(68000L, AscendConstants.getMapBaseRunTimeMs(5));
    }

    @Test
    void getMapUnlockPriceReturnsCorrectValuesForAllOrders() {
        assertEquals(0L, AscendConstants.getMapUnlockPrice(0));
        assertEquals(100L, AscendConstants.getMapUnlockPrice(1));
        assertEquals(500L, AscendConstants.getMapUnlockPrice(2));
        assertEquals(2500L, AscendConstants.getMapUnlockPrice(3));
        assertEquals(10000L, AscendConstants.getMapUnlockPrice(4));
        assertEquals(50000L, AscendConstants.getMapUnlockPrice(5));
    }

    @Test
    void getMapBaseRewardReturnsCorrectValuesForAllOrders() {
        assertEquals(1L, AscendConstants.getMapBaseReward(0));
        assertEquals(5L, AscendConstants.getMapBaseReward(1));
        assertEquals(25L, AscendConstants.getMapBaseReward(2));
        assertEquals(100L, AscendConstants.getMapBaseReward(3));
        assertEquals(500L, AscendConstants.getMapBaseReward(4));
        assertEquals(2500L, AscendConstants.getMapBaseReward(5));
    }

    @Test
    void clampedLookupReturnsLastElementForOutOfBoundsIndex() {
        // Out-of-bounds returns last element
        assertEquals(2500L, AscendConstants.getMapBaseReward(99));
        assertEquals(68000L, AscendConstants.getMapBaseRunTimeMs(99));
        // Negative also returns last element
        assertEquals(2500L, AscendConstants.getMapBaseReward(-1));
        assertEquals(68000L, AscendConstants.getMapBaseRunTimeMs(-1));
    }
    ...
    void getRunnerUpgradeCostIsMonotonicallyIncreasingPerMap() {
        for (int map = 0; map <= 5; map++) {
            BigNumber prev = AscendConstants.getRunnerUpgradeCost(0, map, 0);
            for (int level = 1; level < 20; level++) {
                BigNumber current = AscendConstants.getRunnerUpgradeCost(level, map, 0);
    ...
    void getRunnerUpgradeCostScalesWithLevelAndMap() {
        ...
        BigNumber higher = AscendConstants.getRunnerUpgradeCost(5, 0, 0);
        assertTrue(higher.gt(base));

        // Higher map costs more
        BigNumber higherMap = AscendConstants.getRunnerUpgradeCost(0, 3, 0);
    ...
    void getRunnerMultiplierIncrementScalesWithStars() {
        ...
        BigNumber stars1 = AscendConstants.getRunnerMultiplierIncrement(1);
        assertEquals(0.3, stars1.toDouble(), 1e-6);

        // Stars=5: 0.1 * 3.0^5 = 24.3
        BigNumber stars5 = AscendConstants.getRunnerMultiplierIncrement(5);
    ...
    void getRunnerEntityTypeReturnsCorrectKweebecForStars() {
        assertEquals("Kweebec_Seedling", AscendConstants.getRunnerEntityType(0));
        assertEquals("Kweebec_Sapling", AscendConstants.getRunnerEntityType(1));
        assertEquals("Kweebec_Sproutling", AscendConstants.getRunnerEntityType(2));
        assertEquals("Kweebec_Sapling_Pink", AscendConstants.getRunnerEntityType(3));
        assertEquals("Kweebec_Razorleaf", AscendConstants.getRunnerEntityType(4));
        assertEquals("Kweebec_Rootling", AscendConstants.getRunnerEntityType(5));
        // Out of range: fallback to Sapling
        assertEquals("Kweebec_Sapling", AscendConstants.getRunnerEntityType(6));
        assertEquals("Kweebec_Sapling", AscendConstants.getRunnerEntityType(-1));
    }
    ...
    void getElevationLevelUpCostIsMonotonicallyIncreasing() {
        BigNumber prev = AscendConstants.getElevationLevelUpCost(0);
        for (int level = 1; level <= 400; level++) {
            BigNumber current = AscendConstants.getElevationLevelUpCost(level);
    ...
    void getElevationLevelUpCostBelowSoftCapUsesFirstCurve() {
        ...
        BigNumber cost100 = AscendConstants.getElevationLevelUpCost(100);
    ...
    void getElevationLevelUpCostAboveSoftCapUsesLateCurve() {
        ...
        BigNumber atCap = AscendConstants.getElevationLevelUpCost(300);
        BigNumber afterCap = AscendConstants.getElevationLevelUpCost(301);
        ...
        BigNumber cost400 = AscendConstants.getElevationLevelUpCost(400);
    ...
    void getElevationLevelUpCostWithMultiplier() {
        ...
        BigNumber doubled = AscendConstants.getElevationLevelUpCost(10, BigNumber.fromDouble(2.0));
        BigNumber halved = AscendConstants.getElevationLevelUpCost(10, BigNumber.fromDouble(0.5));
    ...
    void calculateElevationPurchaseReturnsCorrectLevelsAndCost() {
        // Zero volts: 0 levels
        var zero = AscendConstants.calculateElevationPurchase(0, BigNumber.ZERO);
        ...
        for (int i = 0; i < result.levels; i++) {
            expectedCost = expectedCost.add(AscendConstants.getElevationLevelUpCost(i));
        }
    ...
    void calculateElevationPurchaseNeverExceedsAvailableVolt() {
        ...
        for (long budget : budgets) {
            ...
            var result = AscendConstants.calculateElevationPurchase(0, volt);
    ...
    void getElevationMultiplierReturnsLevel() {
        ...
        assertEquals(1.0, AscendConstants.getElevationMultiplier(0), 1e-9);
        assertEquals(1.0, AscendConstants.getElevationMultiplier(1), 1e-9);
        assertEquals(100.0, AscendConstants.getElevationMultiplier(100), 1e-9);
    }

    @Test
    void formatElevationMultiplierFormatsCorrectly() {
        assertEquals("x1", AscendConstants.formatElevationMultiplier(0));
        assertEquals("x5", AscendConstants.formatElevationMultiplier(5));
        assertEquals("x1", AscendConstants.formatElevationMultiplier(1));
    }
    ...
    void voltToXpIsMonotonicallyIncreasing() {
        ...
        for (BigNumber volt : volts) {
            double xp = AscendConstants.voltToXp(volt);
    ...
    void voltToXpAboveDecillionUsesSoftGrowth() {
        ...
        double xpDc = AscendConstants.voltToXp(dc);
        double xpDc10 = AscendConstants.voltToXp(dc10);
    ...
    void xpToVoltIsInverseOfVoltToXp() {
        ...
        for (BigNumber volt : testVolts) {
            double xp = AscendConstants.voltToXp(volt);
            if (xp <= 0) continue;
            BigNumber recovered = AscendConstants.xpToVolt(xp);
    ...
    void getXpForLevelBelowSoftcapUsesQuadratic() {
        ...
        assertEquals(100.0, AscendConstants.getXpForLevel(10), 1e-9);
        assertEquals(10000.0, AscendConstants.getXpForLevel(100), 1e-9);
    }

    @Test
    void getXpForLevelAboveSoftcapUsesQuartic() {
        // At softcap (1000): level^2 = 1,000,000
        double atSoftcap = AscendConstants.getXpForLevel(1000);
        assertEquals(1_000_000.0, atSoftcap, 1e-3);

        // Level 1001: 1001^4 / 1000^2 = ~1,004,006
        double above = AscendConstants.getXpForLevel(1001);
    ...
    void getCumulativeXpForLevelMatchesSumOfIndividualLevels() {
        double sum = 0;
        for (int level = 1; level <= 20; level++) {
            sum += AscendConstants.getXpForLevel(level);
            double cumulative = AscendConstants.getCumulativeXpForLevel(level);
    ...
    void calculateLevelFromXpRoundTripsWithGetCumulativeXp() {
        for (int level : new int[]{1, 10, 50, 100, 500}) {
            double xp = AscendConstants.getCumulativeXpForLevel(level);
            int calculated = AscendConstants.calculateLevelFromXp(xp);
    ...
    void getXpProgressReturnsCurrentAndNextLevelXp() {
        ...
        double cumXp5 = AscendConstants.getCumulativeXpForLevel(5);
        double[] progress = AscendConstants.getXpProgress(cumXp5);
        assertEquals(0.0, progress[0], 1e-6, "At boundary, currentXpInLevel should be 0");
        assertEquals(AscendConstants.getXpForLevel(6), progress[1], 1e-6);

        // Mid-level
        double midXp = cumXp5 + AscendConstants.getXpForLevel(6) / 2;
        double[] midProgress = AscendConstants.getXpProgress(midXp);
