...
- **Global: `/credits` command** - Added a global `/credits` command listing third-party cosmetic/modpack credits used on Hyvexa.
...
- **RunOrFall: Admin-configurable feather reward values** - Added `/rof admin` fields to configure feather gains per minute alive, per elimination, and per win.
- **RunOrFall: Achievement page + progression set** - Added an Ascend-style achievement UI with 16 achievements across `Wins`, `Blocks Broken`, `Time Survived`, and `Blink Usage`, driven by RunOrFall lifetime stats.
