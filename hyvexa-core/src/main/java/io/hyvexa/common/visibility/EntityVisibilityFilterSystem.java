...
/**
 * Filters entities from player visibility based on EntityVisibilityManager.
 * Any entity with a UUIDComponent can be hidden from specific viewers.
 */
public class EntityVisibilityFilterSystem extends EntityTickingSystem<EntityStore> {
    ...
    public void tick(float delta, int entityId, ArchetypeChunk<EntityStore> chunk, Store<EntityStore> store,
        ...
        Set<UUID> hiddenTargets = EntityVisibilityManager.get().getHiddenTargets(viewerRef.getUuid());
