...
public class MedalRewardStore {
    ...
    public void setRewards(String category, int bronze, int silver, int gold) {
        if (category == null) {
            return;
        }
        String key = normalizeCategory(category);
        rewards.put(key, new MedalRewards(Math.max(0, bronze), Math.max(0, silver), Math.max(0, gold)));
        persistToDatabase(key, bronze, silver, gold);
    }
    ...
    private void loadAll() {
        ...
        String sql = "SELECT category, bronze_feathers, silver_feathers, gold_feathers FROM medal_rewards";
        try (Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    rewards.put(normalizeCategory(category), new MedalRewards(bronze, silver, gold));
                }
            }
        } catch (SQLException e) {
    ...
    private void persistToDatabase(String category, int bronze, int silver, int gold) {
        if (!DatabaseManager.getInstance().isInitialized()) {
            return;
        }
        String sql = "INSERT INTO medal_rewards (category, bronze_feathers, silver_feathers, gold_feathers) "
                + "VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE "
                + "bronze_feathers = ?, silver_feathers = ?, gold_feathers = ?";
        try (Connection conn = DatabaseManager.getInstance().getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            DatabaseManager.applyQueryTimeout(stmt);
            ...
            stmt.setInt(4, gold);
            stmt.setInt(5, bronze);
            stmt.setInt(6, silver);
            stmt.setInt(7, gold);
            stmt.executeUpdate();
        } catch (SQLException e) {
    ...
    public static class MedalRewards {
        ...
        public MedalRewards(int bronze, int silver, int gold) {
            this.bronze = bronze;
            this.silver = silver;
            this.gold = gold;
        }
