...
public class MapSelectPage extends BaseParkourPage {
    ...
    private void buildMapList(Ref<EntityStore> ref, Store<EntityStore> store, UICommandBuilder commandBuilder,
        ...
        for (Map map : maps) {
            ...
            if (hasMedalTimes) {
                ...
                buildMedalLabel(commandBuilder, prefix, "Emerald", map.getEmeraldTimeMs(),
                        earned.contains(Medal.EMERALD));
            }
    ...
    private static Medal getHighestMedal(Set<Medal> earned) {
        if (earned.contains(Medal.INSANE)) return Medal.INSANE;
        if (earned.contains(Medal.EMERALD)) return Medal.EMERALD;
