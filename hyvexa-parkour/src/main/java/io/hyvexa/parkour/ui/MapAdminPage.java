...
public class MapAdminPage extends InteractiveCustomUIPage<MapAdminPage.MapData> {
    ...
    private String medalPlatinumSec = "";
    ...
    public void handleDataEvent(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store,
        ...
        if (data.medalBronzeSec != null) {
            medalBronzeSec = data.medalBronzeSec.trim();
        }
        if (data.medalSilverSec != null) {
            medalSilverSec = data.medalSilverSec.trim();
        }
        if (data.medalGoldSec != null) {
            medalGoldSec = data.medalGoldSec.trim();
        }
        if (data.medalPlatinumSec != null) {
            medalPlatinumSec = data.medalPlatinumSec.trim();
        }
        if (data.button == null) {
    ...
    private void handleSelectMap(Ref<EntityStore> ref, Store<EntityStore> store, String mapSelectionId) {
        ...
        if (map != null) {
            ...
            medalPlatinumSec = map.getPlatinumTimeMs() != null ? String.valueOf(map.getPlatinumTimeMs() / 1000) : "";
        }
        sendRefresh(ref, store);
    }
    ...
    private void handleCreate(Ref<EntityStore> ref, Store<EntityStore> store) {
        ...
        Long platinumMs = parseMedalTime(player, medalPlatinumSec, "Platinum");
        if (bronzeMs != null && bronzeMs == -1L) { return; }
        if (silverMs != null && silverMs == -1L) { return; }
        if (goldMs != null && goldMs == -1L) { return; }
        if (platinumMs != null && platinumMs == -1L) { return; }
        ...
        if (platinumMs != null && goldMs != null && platinumMs >= goldMs) {
            player.sendMessage(Message.raw("Platinum time must be less than gold time."));
            return;
        }
        map.setBronzeTimeMs(bronzeMs);
        map.setSilverTimeMs(silverMs);
        map.setGoldTimeMs(goldMs);
        map.setPlatinumTimeMs(platinumMs);
        map.setStart(start);
    ...
    private Long parseMedalTime(Player player, String raw, String label) {
        ...
        try {
            ...
            if (sec <= 0) {
                player.sendMessage(Message.raw(label + " time must be positive."));
                return -1L;
            }
            return sec * 1000L;
        } catch (NumberFormatException ignored) {
            player.sendMessage(Message.raw(label + " time must be a number (seconds)."));
            return -1L;
        }
    }
    ...
    private void populateFields(UICommandBuilder commandBuilder) {
        ...
        commandBuilder.set("#MedalPlatinumField.Value", medalPlatinumSec);
    ...
    private void bindEvents(UIEventBuilder uiEventBuilder) {
        ...
        uiEventBuilder.addEventBinding(CustomUIEventBindingType.ValueChanged, "#MedalBronzeField",
                EventData.of(MapData.KEY_MEDAL_BRONZE, "#MedalBronzeField.Value"), false);
        uiEventBuilder.addEventBinding(CustomUIEventBindingType.ValueChanged, "#MedalSilverField",
        ...
        uiEventBuilder.addEventBinding(CustomUIEventBindingType.ValueChanged, "#MedalPlatinumField",
                EventData.of(MapData.KEY_MEDAL_PLATINUM, "#MedalPlatinumField.Value"), false);
    ...
    public static class MapData {
        ...
        static final String KEY_MEDAL_PLATINUM = "@MedalPlatinum";
        ...
        public static final BuilderCodec<MapData> CODEC = BuilderCodec.<MapData>builder(MapData.class, MapData::new)
                ...
                .addField(new KeyedCodec<>(KEY_MEDAL_PLATINUM, Codec.STRING), (data, value) -> data.medalPlatinumSec = value, data -> data.medalPlatinumSec)
                .build();
